...
export const PurchaseInvoicePDF = ({ inv, settings }) => {
    ...
    return (
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={{ border: '1 solid #000' }}>
                    <View style={styles.row}>
                        ...
                        <View style={[styles.w30, styles.pad, styles.borderRight]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>Invoice No.</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{inv.invoice_number}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>Due Date</Text>
                            <Text style={styles.bold}>{fmtDate(inv.due_date)}</Text>
                        </View>
                        <View style={[styles.w30, styles.pad]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>Invoice Date</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{fmtDate(inv.invoice_date)}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>Status</Text>
                            <Text style={styles.bold}>{(inv.status || 'PENDING').replace('_', ' ').toUpperCase()}</Text>
                        </View>
                    </View>
                    <View style={[styles.borderTop, styles.pad]}>
                        <Text style={{ color: '#555', fontSize: 8 }}>Supplier (Bill From)</Text>
                        <Text style={[styles.bold, { fontSize: 10 }]}>{inv.supplier?.name || 'N/A'}</Text>
                        {inv.supplier?.address_line1 && <Text>{inv.supplier.address_line1}</Text>}
                        {inv.supplier?.gst_number && <Text>GSTIN: <Text style={styles.bold}>{inv.supplier.gst_number}</Text></Text>}
                        {inv.supplier?.phone && <Text>Ph: {inv.supplier.phone}</Text>}
                    </View>
                </View>

                <View style={{ border: '1 solid #000', borderTop: 'none' }}>
                    <View style={styles.tableHeader}>
                        <Text style={[styles.tableCell, styles.center, { width: 30 }]}>#</Text>
                        <Text style={[styles.tableCell, { flex: 1 }]}>Description</Text>
                        <Text style={[styles.tableCell, styles.center, { width: 45 }]}>HSN</Text>
                        <Text style={[styles.tableCell, styles.center, { width: 45 }]}>Qty</Text>
                        <Text style={[styles.tableCell, styles.center, { width: 50 }]}>Rate(₹)</Text>
                        <Text style={[styles.tableCell, styles.center, { width: 55 }]}>Taxable</Text>
                        <Text style={[styles.tableCell, styles.center, { width: 40 }]}>Tax%</Text>
                        <Text style={[styles.tableCellLast, styles.center, { width: 60 }]}>Amount(₹)</Text>
                    </View>
                    ...
                    {items.map((item, i) => {
                        ...
                        return (
                            <View style={styles.row} key={i}>
                                <Text style={[styles.tableCell, styles.center, { width: 30 }]}>{i + 1}</Text>
                                ...
                                <Text style={[styles.tableCell, styles.center, { width: 45 }]}>{fmtN(item.quantity, 0)} {item.product?.unit || ''}</Text>
                                <Text style={[styles.tableCell, styles.right, { width: 50 }]}>{fmtN(item.unit_price)}</Text>
                                <Text style={[styles.tableCell, styles.right, { width: 55 }]}>{fmtN(taxable)}</Text>
                                <Text style={[styles.tableCell, styles.center, { width: 40 }]}>{fmtN(item.tax_percentage)}%</Text>
                                <Text style={[styles.tableCellLast, styles.right, styles.bold, { width: 60 }]}>{fmtN(taxable + tax)}</Text>
                            </View>
                        );
                    })}
                    ...
                    {Array(Math.max(0, 8 - items.length)).fill(0).map((_, idx) => (
                        <View style={styles.row} key={'empty_' + idx}>
                            ...
                            <Text style={[styles.tableCellLast, { width: 60 }]} />
                        </View>
                    ))}
                    ...
                    <View style={[styles.row, styles.borderTop, { backgroundColor: '#f9f9f9' }]}>
                        ...
                        <Text style={[styles.tableCell, styles.right, styles.bold, { width: 55 }]}>{fmtN(items.reduce((a, it) => a + (it.quantity * it.unit_price * (1 - (it.discount_percentage || 0) / 100)), 0))}</Text>
                        <Text style={[styles.tableCell, { width: 40 }]} />
                        <Text style={[styles.tableCellLast, styles.right, styles.bold, { width: 60 }]}>{fmtN(total)}</Text>
                    </View>
                </View>
                ...
                <View style={[styles.row, styles.borderAll, { borderTop: 'none', fontSize: 10 }]}>
                    <Text style={[styles.pad, styles.borderRight, styles.w33]}>Invoice Total: <Text style={styles.bold}>₹{fmtN(inv.total)}</Text></Text>
                    <Text style={[styles.pad, styles.borderRight, styles.w33]}>Already Paid: <Text style={[styles.bold, { color: 'green' }]}>₹{fmtN(inv.paid_amount)}</Text></Text>
                    <Text style={[styles.pad, styles.w33]}>Balance Due: <Text style={[styles.bold, { color: 'red' }]}>₹{fmtN(inv.balance_due)}</Text></Text>
                </View>
                ...
                <View style={[styles.footerSection, { marginTop: 0 }]}>
                    <View style={[styles.pad, styles.borderRight, { flex: 1 }]}>
                        ...
                        {inv.notes && <Text style={{ marginTop: 2, fontSize: 8 }}><Text style={styles.bold}>Notes:</Text> {inv.notes}</Text>}
                        {inv.terms_conditions && <Text style={{ marginTop: 2, fontSize: 8 }}><Text style={styles.bold}>Terms:</Text> {inv.terms_conditions}</Text>}
                    </View>
                    <View style={[styles.pad, styles.center, { width: 200 }]}>
                        ...
                        <Text style={styles.bold}>Accounts / Store Manager</Text>
                    </View>
                </View>

                <Text style={[styles.center, { fontSize: 8, marginTop: 4 }]}>This is a Computer Generated Document</Text>
            </Page>
        </Document>
    );
};
