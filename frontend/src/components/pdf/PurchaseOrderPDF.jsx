...
export const PurchaseOrderPDF = ({ po, settings }) => {
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={{ border: '1 solid #000' }}>
                    <View style={styles.row}>
                        ...
                        <View style={[styles.w30, styles.pad, styles.borderRight]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>PO Number</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{po.po_number}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>Expected Delivery</Text>
                            <Text style={styles.bold}>{fmtDate(po.expected_delivery_date)}</Text>
                        </View>
                        <View style={[styles.w30, styles.pad]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>PO Date</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{fmtDate(po.po_date)}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>Status</Text>
                            <Text style={styles.bold}>{(po.status || '-').toUpperCase()}</Text>
                        </View>
                    </View>
                    <View style={[styles.borderTop, styles.pad]}>
                        <Text style={{ color: '#555', fontSize: 8 }}>Supplier (Bill From)</Text>
                        <Text style={[styles.bold, { fontSize: 10 }]}>{po.supplier?.name || 'N/A'}</Text>
                        {po.supplier?.address_line1 && <Text>{po.supplier.address_line1}</Text>}
                        {po.supplier?.gst_number && <Text>GSTIN: <Text style={styles.bold}>{po.supplier.gst_number}</Text></Text>}
                        {po.supplier?.phone && <Text>Ph: {po.supplier.phone}</Text>}
                    </View>
                </View>
                ...
                <View style={{ border: '1 solid #000', borderTop: 'none' }}>
                    <View style={styles.tableHeader}>
                        ...
                        <Text style={[styles.tableCell, styles.center, { width: 35 }]}>Tax%</Text>
                ...
                <View style={[styles.footerSection, { marginTop: 0 }]}>
                    <View style={[styles.pad, styles.borderRight, { flex: 1 }]}>
                        ...
                        {po.notes && <Text style={{ marginTop: 2, fontSize: 8 }}><Text style={styles.bold}>Notes:</Text> {po.notes}</Text>}
                        {po.terms_conditions && <Text style={{ marginTop: 2, fontSize: 8 }}><Text style={styles.bold}>Terms:</Text> {po.terms_conditions}</Text>}
                    </View>
                    <View style={[styles.pad, styles.center, { width: 200 }]}>
                        ...
                        <Text style={styles.bold}>Purchase Manager</Text>
                    </View>
                </View>

                <Text style={[styles.center, { fontSize: 8, marginTop: 4 }]}>This is a Computer Generated Purchase Order</Text>
            </Page>
        </Document>
    );
};
