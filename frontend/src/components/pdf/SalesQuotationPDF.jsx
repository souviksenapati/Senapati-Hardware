...
export const SalesQuotationPDF = ({ quot, settings }) => {
            ...
            <Page size="A4" style={styles.page}>
                ...
                <View style={{ border: '1 solid #000' }}>
                    <View style={styles.row}>
                        ...
                        <View style={[styles.w30, styles.pad, styles.borderRight]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>Quotation No.</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{quot.quotation_number}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>Valid Until</Text>
                            <Text style={styles.bold}>{fmtDate(quot.valid_until)}</Text>
                        </View>
                        <View style={[styles.w30, styles.pad]}>
                            <Text style={{ color: '#555', fontSize: 8 }}>Date</Text>
                            <Text style={[styles.bold, { marginBottom: 8 }]}>{fmtDate(quot.quotation_date)}</Text>
                            <Text style={{ color: '#555', fontSize: 8 }}>GST Type</Text>
                            <Text style={styles.bold}>{(quot.gst_type || 'CGST/SGST').replace('_', '/').toUpperCase()}</Text>
                        </View>
                    </View>
                    <View style={[styles.borderTop, styles.pad]}>
                        <Text style={{ color: '#555', fontSize: 8 }}>Customer (Bill To)</Text>
                        <Text style={[styles.bold, { fontSize: 10 }]}>{quot.customer?.name || 'N/A'}</Text>
                        {quot.customer?.address_line1 && <Text>{quot.customer.address_line1}</Text>}
                        {quot.customer?.gst_number && <Text>GSTIN: <Text style={styles.bold}>{quot.customer.gst_number}</Text></Text>}
                        {quot.customer?.state && <Text>State: {quot.customer.state}</Text>}
                    </View>
                </View>
                ...
                <View style={[styles.footerSection, { marginTop: 0 }]}>
                    <View style={[styles.pad, styles.borderRight, { flex: 1 }]}>
                        ...
                        {quot.notes && <Text style={{ marginTop: 4, fontSize: 8 }}><Text style={styles.bold}>Notes:</Text> {quot.notes}</Text>}
                        {quot.terms_conditions && <Text style={{ marginTop: 2, fontSize: 8 }}><Text style={styles.bold}>Terms:</Text> {quot.terms_conditions}</Text>}
                    </View>
                ...
                <Text style={[styles.center, { fontSize: 8, marginTop: 4 }]}>This is a Computer Generated Quotation</Text>
            </Page>
        </Document>
    );
};
